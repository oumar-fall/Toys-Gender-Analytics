...
let obj = JSON.parse(fs.readFileSync("./LaGrandeRecreBis/DB.json"));
...
var analyse_marques = function (database) {
  ...
  marques.sort();
  fs.writeFileSync("./LaGrandeRecreBis/marques.csv", "marque_id;marque_name\n");
  for (let id_marque in marques){
      fs.appendFileSync("./LaGrandeRecreBis/marques.csv", id_marque + ';"' + marques[id_marque] + '"\n');
  }
};
...
var createNewCSV = function () {
  ...
  for (let id_jouet in obj) {
    ...
    appendCSV("./LaGrandeRecreBis/newDB.csv", obj[id_jouet]);
  }
};
